...
__all__ = ("RoadSnakeAdapter", "RoadSnakeDetect")
...
class RoadSnakeAdapter(nn.Module):
    ...
    def __init__(
        ...
        self.horizontal_weight = nn.Parameter(torch.full((hidden, kernel_size), 1.0 / kernel_size))
        self.vertical_weight = nn.Parameter(torch.full((hidden, kernel_size), 1.0 / kernel_size))
    ...
    def _sample_curve(
        ...
        positions = self.positions.to(device=device, dtype=dtype).view(1, self.kernel_size, 1, 1)
        ...
        grid = torch.stack(
            (self._normalize_grid(grid_x, width), self._normalize_grid(grid_y, height)), dim=-1
        )
        grid = grid.permute(0, 2, 3, 1, 4).reshape(batch, height, width * self.kernel_size, 2)
        sampled = F.grid_sample(feature, grid, mode="bilinear", padding_mode="border", align_corners=True)
        sampled = sampled.reshape(batch, channels, height, width, self.kernel_size).permute(0, 1, 4, 2, 3)
        return (sampled * weight.to(dtype=dtype).view(1, channels, self.kernel_size, 1, 1)).sum(dim=2)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        horizontal = self.act(self.horizontal_norm(self._sample_curve(reduced, offset_h, horizontal=True)))
        vertical = self.act(self.vertical_norm(self._sample_curve(reduced, offset_v, horizontal=False)))
...
class RoadSnakeDetect(Detect):
    ...
    def __init__(
        ...
        if len(ch) != 3:
            raise ValueError(f"RoadSnakeDetect expects P3/P4/P5 inputs, got {len(ch)} levels")
